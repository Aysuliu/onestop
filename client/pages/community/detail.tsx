...
import { LIKE_TARGET_BOARD_ARTICLE, UPDATE_COMMENT, CREATE_COMMENT } from '../../apollo/user/mutation';
...
const CommunityDetail: NextPage = ({ initialInput, ...props }: T) => {
	...
	const {
		...
		refetch: boardArticleRefetch} = useQuery(GET_BOARD_ARTICLE, {
			...
			onCompleted: (data: any) => {
				setBoardArticle(data?.getBoardArticles);
	...
	const updateButtonHandler = async (commentId: string, commentStatus?: CommentStatus.DELETE) => {
		try {
			...
			if(commentStatus) {
				if(await sweetConfirmAlert("Do you want to confirm update?")){
					await updateComment({
						variables: {
							input: updateData
						}
